...
export class Clear extends AbstractCommand {
    ...
    public async run(msg: Message): Promise<void> {
        ...
        if (count > 99 || count < 1) {
            msg.reply("you can only delete up to 99 message at once!");
            return;
        }

        try {
            msg.channel.bulkDelete(count).then(async () => {
                msg.channel.send(`${count} Nachrichten gelöscht!`).then(async msg => {
